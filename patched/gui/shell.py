...
def apply_theme() -> None:
    ...
    # chronos.css is the single source of truth — no write-on-startup sync.
    try:
        from pathlib import Path

        css_path = Path(__file__).resolve().parent / "static" / "chronos.css"
        ver = int(css_path.stat().st_mtime) if css_path.is_file() else 1
    except Exception:
        ver = 1
    ui.add_head_html(
...
def _status_bar(engine: str) -> None:
    ...
    with ui.element("div").classes("status-bar"):
        ...
        ui.html('<span class="sep" style="width:1px;height:14px;background:var(--border)"></span>', sanitize=False)
        ui.html(
            '<span class="status-pill"><span class="glow-dot"></span>CJIS-aware</span>',
            sanitize=False,
        )

        def _bar_text() -> str:
            # M/D/YY e.g. 7/9/26
            date_str = format_local_date(style="short")
            return (
        ...
        try:
            ui.run_javascript(
                ...
                try {
                  ...
                  if (localStorage.getItem('chronos_dock_hidden') !== '0')
                    s.classList.add('dock-hidden');  // default closed
                } catch (e) {}
                """
            )
        except Exception:
