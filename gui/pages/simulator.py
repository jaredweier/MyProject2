...
def render_simulator() -> None:
    def body() -> None:
        ...
        <style>
          :root {
              --sim-bg: #090e17; /* Deep midnight blue */
              --sim-surface: #131c2f;
              --sim-border: rgba(56, 189, 248, 0.3);
              --sim-border-glow: rgba(56, 189, 248, 0.8);
              --sim-primary: #38bdf8;
              --sim-primary-hover: #7dd3fc;
              --sim-text: #e2e8f0;
              --muted: #94a3b8;
              --accent: #f43f5e;
          }
          ...
          /* Option Cards - The futuristic containers */
          .sim-option-card {
              background: linear-gradient(145deg, var(--sim-surface), #1a263d);
          ...
          .sim-option-card:hover {
              border-color: var(--sim-border-glow);
              box-shadow: 0 12px 48px rgba(56, 189, 248, 0.15), inset 0 1px 0 rgba(255,255,255,0.1);
          ...
          .sim-step-on {
              ...
              box-shadow: 0 0 16px rgba(56, 189, 248, 0.1) inset;
        ...
        with sim_page:
            ...
            with step1:
                with ui.expansion(
                    ...
                    icon="info",
                    value=False,
                ).classes("sim-adv w-full q-mb-sm"):
                    ui.label(
                        "Constraints load from your last saved form (not program defaults). "
                        "Lock a row to fix it; unlocked rows stay free for Find best. "
                        "When you lock a new constraint, a popup may suggest best-fit values "
                        "from what you already chose — you can always type something else. "
                        "Save constraints stores this form for next session."
                    ).style(_HINT)

                lock_progress_host = ui.element("div").classes("sim-progress-strip")

                def _refresh_lock_progress():
                    pairs = [
                        bool(use_rotation.value),
                        bool(use_officers.value),
                        bool(use_length.value),
                        bool(use_annual.value),
                        bool(use_starts.value),
                        bool(use_min_ps.value),
                        bool(use_247.value),
                        bool(use_style.value),
                        bool(use_windows.value),
                        bool(use_nearby.value),
                        bool(use_certs.value),
                        bool(use_fatigue.value),
                        bool(use_flsa.value),
                    ]
                    locked_n = sum(1 for x in pairs if x)
                    total = len(pairs)
                    pct = int(round(100 * locked_n / total)) if total else 0
                    free_n = total - locked_n
                    lock_progress_host.clear()
                    with lock_progress_host:
                        ui.html(
                            '<span class="sim-progress-label">Locks</span>',
                            sanitize=False,
                        )
                        ui.html(
                            f'<span class="sim-progress-value">{locked_n}/{total}</span>',
                            sanitize=False,
                        )
                        ui.html(
                            f'<div class="sim-progress-bar" role="progressbar" '
                            f'aria-valuenow="{pct}" aria-valuemin="0" aria-valuemax="100">'
                            f'<i style="width:{pct}%"></i></div>',
                            sanitize=False,
                        )
                        ui.label(f"{locked_n} locked · {free_n} free for optimizer").classes("text-xs").style(
                            "color:var(--muted)"
                        )
            ...
            with ui.element("div").classes("grid-2"):
                with panel("Coverage Requirements"):
                    ...
                    with (
                        ...
                        use_rotation = ui.checkbox("Rotation Pattern", value=False)
                        with ui.element("div"):
                            rotation = (
                                ui.select(
                                    _ROTATION_OPTIONS,
                    ...
                    with (
                        ...
                        use_officers = ui.checkbox("Officer Count", value=False)
                        with ui.element("div"):
                            officers = ui.input(
                    ...
                    with (
                        ...
                        use_length = ui.checkbox("Shift Length", value=False)
                        with ui.element("div"):
                            length = ui.input(
                    ...
                    with (
                        ...
                        use_annual = ui.checkbox("Annual Hours Target", value=False)
                        with ui.element("div"):
                            annual = ui.input(
                    ...
                    with (
                        ...
                        use_starts = ui.checkbox("Shift Start Times", value=False)
                        with ui.element("div"):
                            starts = ui.input(
                    ...
                    with (
                        ...
                        use_min_ps = ui.checkbox("Minimum Officers Per Shift", value=False)
                        with ui.element("div"):
                            min_ps = ui.input(
                    ...
                    with (
                        ...
                        use_247 = ui.checkbox("24/7 Continuous Minimum", value=False)
                        with ui.element("div"):
                            cov247 = ui.input(
                    ...
                    with (
                        ...
                        use_style = ui.checkbox("Rotation Style / Multi-Block", value=False)
                        with ui.element("div"):
                            rot_style = (
                    ...
                    with (
                        ...
                        use_nearby = ui.checkbox("Nearby Start Bumps (Work Days)", value=False)
                        with ui.element("div"):
                    ...
                    with (
                        ...
                        use_certs = ui.checkbox("Required Cert Codes (Fill Gate)", value=False)
                        with ui.element("div"):
                            cert_codes = ui.input(
                    ...
                    with (
                        ...
                        use_fatigue = ui.checkbox("Fatigue / Rest Rules", value=False)
                        with ui.element("div"):
                            min_rest = ui.input(
                    ...
                    with (
                        ...
                        use_flsa = ui.checkbox("Avoid FLSA Overtime", value=False)
                        with ui.element("div"):
                            flsa_days = ui.input(
                ...
                with panel("Extra Minimum Staffing Windows"):
                    ...
                    use_windows = ui.checkbox("Extra Minimum Staffing Windows", value=False)
        ...
        with step2:
            ...
            with ui.element("div").classes("sim-hero"):
                ui.html(
                    ...
                    "Lock what you know on Requirements; leave free what the optimizer may vary. "
                    "Standard: free officer count near form hint (fast). "
                    "Deep: free officers 4–20 and free length half-hour grid. "
                ...
                with ui.element("div").classes("sim-hero-actions"):
                    ...
                    with ui.element("div").classes("sim-tool-group"):
                        ...
                        search_depth = (
                            ...
                            .tooltip(
                                "Standard: free N near form hint (≈±2). "
                                "Deep: free N 4–20 + denser free lengths (slower). "
            ...
            with ui.expansion(
                "Tools: export, fairness, sensitivity, slots",
                icon="build",
            ).classes("sim-adv w-full"):
                with ui.row().classes("gap-2 flex-wrap q-mt-sm"):
                    btn_export_opts = (
                        ui.button("Export options CSV").classes("btn-ghost").props("no-caps outline dense")
                    )
                    btn_audit = ui.button("Export search audit").classes("btn-ghost").props("no-caps outline dense")
                    btn_memo = ui.button("Staffing memo").classes("btn-ghost").props("no-caps outline dense")
                    btn_diff = ui.button("Diff A vs B").classes("btn-ghost").props("no-caps outline dense")
                    btn_fair = ui.button("Fairness report").classes("btn-ghost").props("no-caps outline dense")
                    btn_plain = ui.button("Plain English explain").classes("btn-ghost").props("no-caps outline dense")
                    btn_sens = ui.button("Sensitivity (+N / night)").classes("btn-ghost").props("no-caps outline dense")
                    btn_live_con = (
                        ui.button("Import live constraints").classes("btn-ghost").props("no-caps outline dense")
                    )
                    btn_apply_month = (
                        ui.button("Apply winner → draft month").classes("btn-primary").props("no-caps unelevated dense")
                    )
                    btn_cpsat = ui.button("CP-SAT (small N)").classes("btn-ghost").props("no-caps outline dense")
                    btn_seed = ui.button("Lock selected as seed").classes("btn-ghost").props("no-caps outline dense")
                    btn_copy = ui.button("Copy summary").classes("btn-ghost").props("no-caps outline dense")
                    btn_heat = ui.button("Weekend heat").classes("btn-ghost").props("no-caps outline dense")
                    btn_export_cfg = ui.button("Export config JSON").classes("btn-ghost").props("no-caps outline dense")
                    btn_import_cfg = ui.button("Import config JSON").classes("btn-ghost").props("no-caps outline dense")
                    btn_pin = ui.button("Pin selected").classes("btn-ghost").props("no-caps outline dense")
                    btn_pins = ui.button("Pinned list").classes("btn-ghost").props("no-caps outline dense")
                    btn_heat_png = ui.button("Heat map").classes("btn-ghost").props("no-caps outline dense")
                    btn_win_drill = ui.button("Window failures").classes("btn-ghost").props("no-caps outline dense")
                    btn_share = ui.button("Share best").classes("btn-ghost").props("no-caps outline dense")
                    btn_slot_a = ui.button("Save → A").classes("btn-ghost").props("no-caps outline dense")
                    btn_slot_b = ui.button("Save → B").classes("btn-ghost").props("no-caps outline dense")
                    btn_slot_c = ui.button("Save → C").classes("btn-ghost").props("no-caps outline dense")
                    btn_slots = ui.button("Open A/B/C").classes("btn-ghost").props("no-caps outline dense")
        ...
        def _apply_suggest_values(values: dict) -> None:
            ...
            try:
                if "use_rotation" in values:
                    use_rotation.value = bool(values["use_rotation"])
                ...
                if "use_style" in values:
                    use_style.value = bool(values["use_style"])
                ...
                if "use_nearby" in values:
                    use_nearby.value = bool(values["use_nearby"])
        ...
        # Attach after helpers exist
        try:
            with step1:
                ui.button(
                    "Suggest Next Constraint",
                    on_click=_suggest_next_unlocked,
                ).classes("btn-ghost q-mt-sm").props("no-caps outline")
        except Exception:
            pass
        ...
        def _optimizer_kwargs(*, require_hard_ok: bool) -> dict:
            ...
            depth = str(state.get("search_depth") or "standard").lower()
            deep = depth in ("deep", "full", "exhaustive")
            if not free_officers:
                off_counts = [int(n)]
            elif deep:
                # Deep free headcount: full 4–20 band
                off_counts = list(range(4, 21))
            else:
                # Standard (UAT): band near form hint / real-world 7–9 path
                try:
                    hint = int(n) if n and n >= 1 else int((officers.value or "8").strip() or "8")
                except (TypeError, ValueError):
                    hint = 8
                hint = max(4, min(20, hint))
                lo = max(4, hint - 2)
                hi = min(16, max(hint + 2, lo + 2))
                off_counts = list(range(lo, hi + 1))
            # Free length: standard = LE common lengths; deep = half-hour grid
            length_opts = None
            if free_lengths:
                length_opts = [8.0, 8.5, 9.0, 9.5, 10.0, 10.5, 11.0, 11.5, 12.0] if deep else [8.0, 10.0, 12.0]
        ...
        def _show_no_match_dialog(
            ...
            with (
                ...
                if sugs:
                    ui.label("Suggested Relaxations").style("color:#86efac;font-weight:600;margin-top:4px")
                    for s in sugs[:3]:
                        ui.label(f"{s.get('rank', 1)}. {s.get('action', '')}").style("color:#E8EDF4;font-size:0.95rem;")
        ...
        btn_export_opts.on_click(export_options)
        btn_audit.on_click(export_audit)
        btn_memo.on_click(export_memo)
        btn_diff.on_click(run_diff_ab)
        btn_fair.on_click(run_fairness)
        btn_plain.on_click(run_plain_explain)
        btn_sens.on_click(run_sensitivity)
        btn_live_con.on_click(run_import_live)
        btn_apply_month.on_click(run_apply_winner_month)
        btn_cpsat.on_click(run_cpsat_small)
        btn_seed.on_click(lock_selected_seed)
        btn_copy.on_click(copy_summary)
        btn_heat.on_click(show_weekend_heat)
        btn_export_cfg.on_click(export_config)
        btn_import_cfg.on_click(import_config)
        btn_pin.on_click(do_pin)
        btn_pins.on_click(show_pins)
        btn_heat_png.on_click(do_heat)
        btn_win_drill.on_click(do_window_drill)
        btn_share.on_click(do_share)
        btn_slot_a.on_click(lambda: save_slot("A"))
        btn_slot_b.on_click(lambda: save_slot("B"))
        btn_slot_c.on_click(lambda: save_slot("C"))
        btn_slots.on_click(show_slots)
