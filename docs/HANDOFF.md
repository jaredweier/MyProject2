...
- **2026-07-21 latest:** Simulator crash fixes (5 dropped/forward-ref bugs from the module split), constraint math bugs (fatigue rest-gap, weight coupling, dead validator, silent defaults), search-space pruning gap (presets never got cheap-rejected), UI regrouped into 6 sections + reworded, officer count now supports custom range, annual-hours math now calendar-independent, FLSA now derived from rotation length. Full detail: `logs/NEXT_SESSION_BRIEF.md`. **Open question: user referenced `Chronos UI Evaluation.dc.html` (a Claude-Design overhaul spec) — not found anywhere searched, awaiting user reply on its location. Do not implement a redesign without it.**
- **Process hygiene (binding):** any process you start for a task (dev server, test run, one-off script) must be terminated once it's no longer needed — do not leave it running after the task that needed it is done. Before starting a new one, check for stray/duplicate processes from earlier work (`Get-CimInstance Win32_Process -Filter "Name='python.exe'"` in PowerShell) and clean up anything abandoned. This session found **20+ stray Python processes** (duplicate `dev.py verify --tier check` and `unittest discover` runs, hung for 3+ hours) plus 2 leftover `Chronos Command GPT` backend processes on port 8000, all silently consuming CPU and causing unrelated live-UI testing to appear broken/flaky. All were killed 2026-07-17 late evening — don't recreate this pile-up.
...
- **Full brief:** `logs/NEXT_SESSION_BRIEF.md` — 3 simulator/optimizer bugs found and fixed here, none yet verified end-to-end. Read it before touching `gui/pages/simulator.py` or `gui/app.py` again.
...
**Last updated:** 2026-07-17 late evening (workspace moved to Claude Chronos Command · 3 simulator/optimizer bugs found + fixed, unverified · connection-leak refactor 29/31 files done in MyProject)
